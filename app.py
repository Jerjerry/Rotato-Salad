...
def generate_print_friendly_html(date: str, schedule: Dict[str, List[str]], down_stations_data: Dict[str, List[int]]) -> str:
    # This function remains identical to the version in station_rotation_final_v4/v5/v6
    html = f"""
    <!DOCTYPE html>
    <html lang='en'>
    <head>
    ...
    <body>
        <div class='container'>
            ...
            <div class='columns'>
                <div class='column'>
    """
    for line_code in LINES_FIRST_COLUMN_HTML: 
    ...
    for line_code in LINES_SECOND_COLUMN_HTML: 
        html += f'<div class="line-group"><div class="line-title">Line {line_code}</div><div class="pairs">'
        has_content_for_line = False
        if schedule.get(line_code):
            for pair_text in schedule[line_code]:
                html += f'<div class="pair">{pair_text}</div>'
            has_content_for_line = True
...
def main() -> None:
    ...
    with st.form(key="station_config_form", clear_on_submit=False):
        ...
        col_widths = [2, 5] # [width_for_label_column, width_for_widget_column]

        for line_key in LINES: 
            if line_key == 'C':
                # --- Line C Accommodations ---
                col_label_c_acc, col_widget_c_acc = st.columns(col_widths)
                with col_label_c_acc:
                    st.markdown(f"**Line C**<br>Accommodations", unsafe_allow_html=True)
                with col_widget_c_acc:
                    st.multiselect(
                        label="", # Empty label, as visual label is in the left column
                        # label_visibility="collapsed", # REMOVED to allow help icon
                        options=all_stations_for_multiselect,
                        key="accommodation_stations_c", 
                        help="Select stations for operators who will remain at their current station (e.g., for '1-1' type pairings)."
                    )
                
                # --- Line C Unavailable ---
                col_label_c_unav, col_widget_c_unav = st.columns(col_widths)
                with col_label_c_unav:
                    st.markdown(f"Unavailable", unsafe_allow_html=True) 
                with col_widget_c_unav:
                    currently_accommodated_c: List[int] = st.session_state.get("accommodation_stations_c", [])
                    options_for_unavailable_c: List[int] = [s for s in all_stations_for_multiselect if s not in currently_accommodated_c]
                    
                    if options_for_unavailable_c: 
                        st.multiselect(
                            label="", # Empty label
                            # label_visibility="collapsed", # REMOVED
                            options=options_for_unavailable_c,
                            key=f"non_op_{line_key}", 
                            help="Select stations on Line C that are broken or cannot be used today. Cannot be an accommodated station."
                        )
                    elif currently_accommodated_c: 
                         st.info(f"All stations are 'Accommodated' or no others to mark 'Unavailable'.")
                    else: 
                        st.warning(f"No stations to mark as 'Unavailable'.")
                
            else: # For all other lines (B, L, M, N, O)
                col_label, col_widget = st.columns(col_widths)
                with col_label:
                    st.markdown(f"**Line {line_key}**<br>Unavailable", unsafe_allow_html=True)
                with col_widget:
                    st.multiselect(
                        label="", # Empty label
                        # label_visibility="collapsed", # REMOVED
                        options=all_stations_for_multiselect,
                        key=f"non_op_{line_key}", 
                        help=f"Select stations on Line {line_key} that are broken or cannot be used today."
                    )
            
            st.write("") # Consistent vertical gap after each line's full configuration
